...
class Timer:
    ...
    def tick(self, tickNbr):
        if self._running and self.__tickCount >= 0:
            logger.info("Timer - tick: {ticksCount} of {deviceTime}"
                        .format(ticksCount=self.__tickCount,
                                deviceTime=self.__quantum))
    ...
    def reset(self, ajuste=False):
        self.__tickCount = self.__quantum - (1 if ajuste else 0)
...
class Hardware:
    def setup(self, memorySize):
        ...
        self._clock.addSubscribers([self._ioDevice, self._timer, self._cpu])
